...
public class BallPanel extends JPanel implements MouseListener, KeyListener{
	...
	public BallPanel(int _width, int _height) {
		...
		for(int i=0;i<5;i++) {
			...
			this.balls.add(new Ball(x, y, radius, speed, angleInDegree, 'a'));
		}
...
//					ball.collideBall(balls);
	...
	public void addBall(int x, int y, char c) {
		int radius = new Random().nextInt(20, 70);
		int speed = new Random().nextInt(3, 7);
		
		balls.add(new Ball(x, y, radius, speed, angleInDegree(), c));
	}
	...
	public void keyPressed(KeyEvent e) {
		...
		if(getkey >= 97 && getkey <= 122 || getkey >= 65 && getkey <= 90 || getkey >=48 && getkey <= 57) {
			int x = new Random().nextInt(10, 600);
			int y = new Random().nextInt(10, 500);
			addBall(x, y, key);
		}
	}
